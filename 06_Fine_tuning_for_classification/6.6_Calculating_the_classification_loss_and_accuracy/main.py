...
# Convert the outputs (logits) into probability scores via the softmax function
# and then obtain the index position of the largest probability value via the argmax function
probas = torch.softmax(outputs[:, -1, :], dim=-1)
label = torch.argmax(probas)
print("Class label:", label)

# Softmax is optional because the largest outputs correspond to the largest probability score
logits = outputs[:, -1, :]
label = torch.argmax(logits)
print("Class label:", label.item())

# For reproducibility due to the shuffling in the training data loader, also the seed is consumed
# at the top of this script when updating the linear output layer to 2 classes. Therefore the seed needs to be reset.
torch.manual_seed(123)

# Apply the calc_accuracy_loader function to calculate the classification accuracies for the different datasets
...
# Apply the calc_loss_loader to calculate the loss scores for the different datasets
